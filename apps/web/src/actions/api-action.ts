...
/**
 * Standard interface for Server Action responses.
 */
...
export async function apiAction<T>({
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.error(
        `HTTP error returned. Status: ${response.status}`,
        logContext,
        errorText,
      );
      ...
      return {
        success: false,
        message:
          "No se pudo procesar la solicitud en el servidor. Por favor, intenta de nuevo.",
    ...
    if (!json.success) {
      logger.warn(
        "API request failed validation/business logic",
        logContext,
